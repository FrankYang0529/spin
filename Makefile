...
test-integration:
	RUST_LOG=$(LOG_LEVEL) cargo test integration_tests::test_dependencies --no-fail-fast -- --nocapture
	RUST_LOG=$(LOG_LEVEL) cargo test integration_tests --no-fail-fast -- --skip integration_tests::test_dependencies --nocapture --include-ignored
